(async function () {
    ...
    const encodeFile = async f => {
        const m = {mimeType: f.type || null, name: f.name ?? null, size: f.size};
        if (f.size > maxImageBytes) return {...m, omitted: 'too-large', limitBytes: maxImageBytes};
        return {...m, encoding: 'base64', dataBase64: await toBase64(f)}
    };
    try {
        const data = await new Promise((resolve, reject) => {
            ...
            req.onsuccess = async e => {
                ...
                try {
                    ...
                    let encoded = 0, omitted = 0, bytes = 0;
                    for (const rec of exp.stores['chat-images']?.data ?? []) {
                        if (rec?.file instanceof Blob) {
                            const enc = await encodeFile(rec.file);
                            rec.file = enc;
                            if (enc.dataBase64) {
                                encoded++;
                                bytes += enc.size
                    ...
                    resolve({encoded: encoded, omitted: omitted, bytes: bytes})
                } catch (err) {
                    db.close();
                    reject(err)
                }
        ...
        alert('Export successful! File downloaded.' + (data.encoded ? ' ' + data.encoded + ' image(s) as base64 (' + data.bytes + ' bytes)' : '') + (data.omitted ? ' ' + data.omitted + ' omitted (too large)' : ''))
    } catch (err) {
